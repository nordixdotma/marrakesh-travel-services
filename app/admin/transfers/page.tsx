...
import { Car, Plus, Search, Eye, Pencil, Trash2, MapPin, DollarSign } from "lucide-react"
...
export default function AdminTransfersPage() {
  ...
  const filteredOffers = useMemo(() => {
    if (!searchQuery.trim()) return transfersOffers
    return transfersOffers.filter(
      (offer) =>
  ...
  const handleCreate = () => {
    alert("Create Transfer functionality coming soon!")
  }

  const handleView = (id: string) => {
    alert(`View details for transfer: ${id}`)
  }

  const handleEdit = (id: string) => {
    alert(`Edit transfer: ${id}`)
  }

  const handleDelete = (id: string) => {
    if (confirm("Are you sure you want to delete this transfer?")) {
      alert(`Delete transfer: ${id}`)
    }
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage your transfers. {transfersOffers.length} total transfers.
          </p>
        </div>
