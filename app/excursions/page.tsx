...
function ExcursionsContent() {
  ...
  // Fetch excursions from backend
  useEffect(() => {
    const fetchExcursions = async () => {
      try {
        setIsLoading(true)
        setError(null)
        const response = await offersApi.getOffers('EXCURSIONS', language)
        
        // Transform backend data to match frontend Offer format
        const transformedExcursions: Offer[] = (response.offers || []).map((backendOffer: any) => {
          ...
          return {
            ...
            detailedDescription: {
              ...
              itinerary: [],
              tips: [],
              duration: backendOffer.excursionDetails?.duration || '',
              difficulty: backendOffer.excursionDetails?.difficulty || '',
              groupSize: '',
            },
            mainImage: mainImage,
            thumbnailImages: [],
        ...
        setAllOffers(transformedExcursions)
        
        // Apply city filter if present
        if (cityParam) {
          setOffers(transformedExcursions.filter(o => o.departCity === cityParam))
        } else {
          setOffers(transformedExcursions)
        }
      } catch (err) {
        const apiError = err as ApiError
        const errorMessage = apiError.message || 'Failed to load excursions'
