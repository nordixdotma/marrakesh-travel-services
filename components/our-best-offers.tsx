  ...
  useEffect(() => {
    const fetchBestOffers = async () => {
      try {
        ...
        const [toursResponse, excursionsResponse, activitiesResponse, transfersResponse, packagesResponse] = await Promise.all([
          offersApi.getOffers('TOURS', language),
          offersApi.getOffers('EXCURSIONS', language),
          offersApi.getOffers('ACTIVITIES', language),
          offersApi.getOffers('TRANSFERS', language),
          offersApi.getOffers('PACKAGES', language),
        ])
        ...
        // Get first 3 offers from each category
        const allBestOffers: Offer[] = [
          ...(toursResponse.offers || []).slice(0, 3).map((offer: any) => transformOffer(offer, 'tours')),
          ...(excursionsResponse.offers || []).slice(0, 3).map((offer: any) => transformOffer(offer, 'excursions')),
          ...(activitiesResponse.offers || []).slice(0, 3).map((offer: any) => transformOffer(offer, 'activities')),
          ...(transfersResponse.offers || []).slice(0, 3).map((offer: any) => transformOffer(offer, 'transfers')),
          ...(packagesResponse.offers || []).slice(0, 3).map((offer: any) => transformOffer(offer, 'packages')),
        ]
